...
export class LoginComponent implements OnInit {
  ...
  onSubmitLogin() {
    if (this.loginForm.valid) {
      this.spinner = true;
      let _username = this.encr.JS_ENCRYPT_STRING(
        this.loginForm.getRawValue().username,
      );
      let _password = this.encr.JS_ENCRYPT_STRING(
      ...
      this.http
        ...
        .then(
          (res: any) => {
            if (res.flag) {
              this.http
                .PostRequest(this.apiurl.GetUserDetail, data)
                .then(
                  (res: any) => {
                    if (res.flag) {
                      ...
                      sessionStorage.setItem(
                        "user_detail",
                        JSON.stringify(res.user_detail),
                      );
  ...
  onSubmitForgot() {
    ...
    this.http
      .PostRequest(this.apiurl.Check_EmpNo_Email, data)
      .then(
        (res) => {
          if (res.flag) {
  ...
  f_sendResetPasswordUrl() {
    ...
    this.http
      .PostRequest(this.apiurl.SendOTP_ForgotPassword, data)
      .then(
        (res) => {
          if (res.flag) {
