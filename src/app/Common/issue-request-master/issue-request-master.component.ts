...
uploadedFiles: any[] = [];
 ...
 this.form = this.formBuilder.group({
      ...
      REQUEST_DATE: [{ value: '', disabled: this.viewflag }],
...
GETISSUERAISEDDETAILSBYISSUENO() {
  ...
  this.http.PostRequest(this.apiurl.GETISSUERAISEDDETAILSBYISSUENO, data).then((res: any) => {
    ...
    this.form.patchValue({
      ...
      RAISED_BY: response.RAISED_BY ===  this.userData[0].LOGIN_ID ? 'SELF' : 'OTHER',
      OTHER_NAME: response.RAISED_BY !== this.userData[0].LOGIN_ID ? response.REQUESTER_NAME : '',
