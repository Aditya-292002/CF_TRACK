...
export class AttendanceProcessApproveComponent implements OnInit {
  ...
  GetEmployeeMonthCommonList() {
    this.spinner = true;
    this.http.PostRequest(this.apiUrl.GetEmployeeMonthCommonList, {}).then(
      (res) => {
        if (res.flag) {
  ...
  filterMonth() {
    if (
      this.form.getRawValue().PYEAR != "" &&
      this.form.getRawValue().PYEAR != null
    ) {
      this._month_list = [];
  ...
  filterCalDays() {
    if (
      this.form.getRawValue().PERIOD_NO != "" &&
      this.form.getRawValue().PERIOD_NO != null
    ) {
      // this._month_list = [];
      this.month_list.forEach((element) => {
        if (
          Number(element.PERIOD_NO) == Number(this.form.getRawValue().PERIOD_NO)
        ) {
  ...
  GetApproveAttendanceProcessDetail() {
    ...
    this.http
      .PostRequest(this.apiUrl.GetApproveAttendanceProcessDetail, data)
      .then(
        (res) => {
          if (res.flag == 0) {
  ...
  SaveApproveAttendanceProcess() {
    ...
    if (this.form.valid) {
      ...
      console.log(data);
      this.http
        .PostRequest(this.apiUrl.SaveApproveAttendanceProcess, data)
        .then(
          (res) => {
            if (res.flag) {
  ...
  GetAttendanceNTSDetailForEMP() {
    ...
    this.http.PostRequest(this.apiUrl.GetAttendanceNTSDetailForEMP, data).then(
      (res) => {
        if (res.flag) {
