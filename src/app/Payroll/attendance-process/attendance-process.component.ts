...
export class AttendanceProcessComponent implements OnInit {
  ...
  filterCalDays(){
    if(this.form.getRawValue().PERIOD_NO != "" && this.form.getRawValue().PERIOD_NO != null){
      this.month_list.forEach(element => {
        if(element.PERIOD_NO == this.form.getRawValue().PERIOD_NO){
  ...
  GetAttendanceProcessDetail(){
    ...
    this.http.PostRequest(this.apiUrl.GetAttendanceProcessDetail, data).then(res => {
      if (res.flag == 0) {
  ...
  onProcessChange(){
    this.GetAttendanceProcessDetail();
  }

  onSaveAttendance(){
    this.SaveAttendanceProcess();
  }
  ...
  SaveAttendanceProcess(){
    ...
    if(this.form.valid){
      this.spinner = true;
        ...
        this.http.PostRequest(this.apiUrl.SaveAttendanceProcess, data).then(res => {
          if (res.flag) {
