...
export class TaxDeclarationComponent implements OnInit {
  ...
  form: FormGroup;
  items_List = [
    { HSG_LOAN: 1, HSG_DESC: "Yes" },
    { HSG_LOAN: 0, HSG_DESC: "No" },
  ];
  Tax_Regime_List = [
    { REGIME_CODE: 1, REGIME_DESC: "NEW" },
    { REGIME_CODE: 0, REGIME_DESC: "Old" },
  ];
  Rent_List = [
    { PAY_RENT: 1, RENT_DESC: "Yes" },
    { PAY_RENT: 0, RENT_DESC: "No" },
  ];
  HSG_LOAN: any;
  REGIME_CODE: any = 1;
  PAY_RENT: any;
  ...
  ngOnInit() {
    ...
    let user_detail = sessionStorage.getItem("user_detail")
      ? JSON.parse(sessionStorage.getItem("user_detail"))
      : [];
    this.LOAN_SANCTION_DATE = this.datepipe.transform(
      new Date(this.LOAN_SANCTION_DATE),
      "dd-MMM-yyyy"
    );
    //console.log('user_detail', user_detail)
    this.EMPLOYEE_NAME =
      user_detail[0].EMP_CODE + " - " + user_detail[0].USER_NAME;
  ...
  GetTaxDeclarationCommonList() {
    ...
    this.http.PostRequest(this.apiUrl.GetTaxDeclarationCommonList, data).then(
      (res) => {
        if ((res.flag = true)) {
  ...
  GetTaxDeclrationDetailList() {
    ...
    this.http.PostRequest(this.apiUrl.GetTaxDeclrationDetailList, data).then(
      (res) => {
        console.log("res", res);
        ...
        if (this.Tax_Dec_Header_List.length > 0) {
          ...
          this.EMPLOYER_TAXABLE =
            this.Tax_Dec_Header_List[0].PREV_EMPLOYER_TAXABLE;
          this.EMPLOYER_TAX_PAID =
            this.Tax_Dec_Header_List[0].PREV_EMPLOYER_TAXPAID;
          ...
          this.LOAN_SANCTION_DATE =
            this.Tax_Dec_Header_List[0].LOAN_SANCTION_DATE;
          this.LOAN_SANCTION_DATE = this.datepipe.transform(
            new Date(this.LOAN_SANCTION_DATE),
            "dd-MMM-yyyy"
          );
  ...
  f_saveFormData(drft: boolean) {
    ...
    this.v_save_data.TAX_DECLARATION_HEADER = [
      {
        ...
        NO_CHILDREN:
          this.REGIME_CODE == 1 || this.NUMBER_OF_CHILDREN == undefined
            ? 0
            : +this.NUMBER_OF_CHILDREN,
        PAY_RENT:
          this.REGIME_CODE == 1 || this.PAY_RENT == undefined
            ? ""
            : this.PAY_RENT,
        ANNUAL_RENT:
          this.REGIME_CODE == 1 || this.ANNUAL_RENT == undefined
            ? 0
            : +this.ANNUAL_RENT,
        HSG_LOAN:
          this.REGIME_CODE == 1 || this.HSG_LOAN == undefined
            ? ""
            : this.HSG_LOAN,
        LOAN_SANCTION_DATE:
          this.REGIME_CODE == 1 || toDate == undefined ? "" : toDate,
        LOAN_SANCTION_VALUE:
          this.REGIME_CODE == 1 || this.LOAN_AMOUNT == undefined
            ? 0
            : +this.LOAN_AMOUNT,
        PROPERTY_VALUE:
          this.REGIME_CODE == 1 || this.VALUE_AGREEMENT == undefined
            ? 0
            : +this.VALUE_AGREEMENT,
        NO_HOUSES:
          this.REGIME_CODE == 1 || this.NUMBER_HOUSE_OWNED == undefined
            ? 0
            : +this.NUMBER_HOUSE_OWNED,
        HSGLOAN_INT:
          this.REGIME_CODE == 1 || this.LOAN_INTEREST_ANNUAL == undefined
            ? 0
    ...
    this.http
      .PostRequest(this.apiUrl.SaveTaxDeclaration, this.v_save_data)
      .then(
        (res) => {
          if (res.flag == true) {
