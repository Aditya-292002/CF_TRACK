...
export class UserRoleChangeComponent implements OnInit {
  ...
  GetUserRoleCommonList() {
    console.log('get leave common list ', this.sharedService.loginUser[0].EMP_CODE);
    let data = {
      LISTTYPE: "all",
    ...
    //return
    this.http.PostRequest(this.apiUrl.GetUserRoleCommonList, data).then(res => {
      if (res.flag) {
...
//return
    this.http.PostRequest(this.apiUrl.GetUserRoleDetails, data).then(res => {
      if (res.flag) {
  ...
  updateRole() {
   // console.log('kk',this.form.getRawValue().ROLE_CODE);
    ...
    if(this.form.getRawValue().ROLE_CODE=='' && this.form.getRawValue().EMP_NO==''){
      this.toast.error("Please select employee");
      return;
    }
    if(this.form.getRawValue().ROLE_CODE==null || this.form.getRawValue().ROLE_CODE==''){
   ...
   // console.log('updateRole', data);
    //return 
    this.http.PostRequest(this.apiUrl.UpdateUserRoleDetails, data).then(res => {
      if (res.flag) {
        //console.log(res);
