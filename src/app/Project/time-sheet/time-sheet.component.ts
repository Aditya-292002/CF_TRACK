...
export class TimeSheetComponent implements OnInit {
  ...
  constructor(private sharedService: SharedServiceService,
    ...
    hours : Array<any> = ["00","01","02","03","04","05","06","07","08","09","10","11","12"]
  ...
  GetTimeSheetCommonList() {
    ...
    this.http.PostRequest(this.apiUrl.GetTimeSheetCommonList, data).then(res => {
      if (res.flag) {
  ...
  f_changeProject(projcode: string = '',index: number = null){
    ...
    this.timesheet_detail[index].TASKID = this.timesheet_detail[index].TASKS.length?this.timesheet_detail[index].TASKS[0].TASKID:''
    this.timesheet_detail[index].ACTIVITY_TYPE = this.task_type_list.length?this.task_type_list[0].TASKTYPE_CODE:''
    setTimeout(() => {
      $('.selectpicker').selectpicker('refresh').trigger('change');
    }, 150);
  }
  ...
  filterTasks(){
    ...
    this.new_data.TASK_ID = this._task_list.length?this._task_list[0].TASKID:''
    this.new_data.TASKTYPE_CODE = this.task_type_list.length?this.task_type_list[0].TASKTYPE_CODE:''
  ...
  f_getDate(){
    let data = {  }
    this.http.PostRequest(this.apiUrl.Get_Min_Timesheet_Date, data).then(res => {
      if (res.flag) {
  ...
  onChangeDate(para: number = null) {
    ...
    this.http.PostRequest(this.apiUrl.GetTimeSheetDetail, data).then(res => {
      if (res.flag) {
        ...
        this.spinner = false;
        if(cur_total_min >= (this.TOTAL_ALLOW_TIME >0?this.TOTAL_ALLOW_TIME:720)){
        ...
        this.H_TIME_IN = res.timesheet_header.length?res.timesheet_header[0].TIME_IN:'';
        this.H_TIME_OUT = res.timesheet_header.length?res.timesheet_header[0].TIME_OUT:'';
        this.TOTAL_ALLOW_TIME = res.timesheet_header.length?res.timesheet_header[0].TOTAL_ALLOW_TIME:'';
        this.timesheet_detail = res.timesheet_detail;

        this.ATT_STATUS = res.timesheet_header.length?res.timesheet_header[0].ATTN_STATUS:'';
        let cur_total_min = 0;
        for (let data of this.timesheet_detail) {
          cur_total_min = cur_total_min + data.TOTAL_MIN;
          data.TOTAL_TIME = this.f_M_H(data.TOTAL_MIN)
  ...
  this.new_data= { TIME_MINUTE: '', PROJECT: '',  TASK: '',  TASK_ID: '', TYPE: '',  TASKTYPE_CODE:'', TIME: '', HH:'00',  MM:'00',REMARK: '' }
  ...
  return false;
    ...
    this.new_data.TASK = this._task_list.find(e => e.TASKID == this.new_data.TASK_ID) 
    this.new_data.TYPE = this.task_type_list.find(e => e.TASKTYPE_CODE == this.new_data.TASKTYPE_CODE)

    let tm = this.new_data.TIME.split(':')
    this.new_data.TIME = ("00" + tm[0].toString()).slice(-2) + ':' + (tm.length>1?("00" + tm[1].toString()).slice(-2):"00")
    ...
    this.timesheet_detail.push({
      SRNO:0,
      PROJ_NAME: this.new_data.PROJECT == "" ? "" : typeof(this.new_data.PROJECT)=='string'?JSON.parse(this.new_data.PROJECT).PROJ_NAME:this.new_data.PROJECT.PROJ_NAME,
      PROJ_CODE: this.new_data.PROJECT == "" ? "" : typeof(this.new_data.PROJECT)=='string'?JSON.parse(this.new_data.PROJECT).PROJ_CODE:this.new_data.PROJECT.PROJ_CODE,
      TASKID: this.new_data.TASK == "" ? "" : typeof(this.new_data.TASK)=='string'?JSON.parse(this.new_data.TASK).TASKID:this.new_data.TASK.TASKID,
      TASK_ID: this.new_data.TASK == "" ? "" : typeof(this.new_data.TASK)=='string'?JSON.parse(this.new_data.TASK).TASKID:this.new_data.TASK.TASKID,
      TASK_DESC: this.new_data.TASK == "" ? "" : typeof(this.new_data.TASK)=='string'?JSON.parse(this.new_data.TASK).TASK_DESC:this.new_data.TASK.TASK_DESC,
      ACTIVITY_TYPE: this.new_data.TYPE == "" ? "" : typeof(this.new_data.TYPE)=='string'?JSON.parse(this.new_data.TYPE).TASKTYPE_CODE:this.new_data.TYPE.TASKTYPE_CODE,
      TASKTYPE_DESC: this.new_data.TYPE == "" ? "" : typeof(this.new_data.TYPE)=='string'?JSON.parse(this.new_data.TYPE).TASKTYPE_DESC:this.new_data.TYPE.TASKTYPE_DESC,
  ...
  calculateTotalTime(){
    ...
    for(let data of this.timesheet_detail){
      if(data.TOTAL_MIN != null && data.TOTAL_MIN != '' && data.ISDELETED != 1){
        this.total_minutes = this.total_minutes + data.TOTAL_MIN;
      }
    }
    this.total_hrs_time = this.f_M_H(this.total_minutes)
    if(this.total_minutes >= (this.TOTAL_ALLOW_TIME >0?this.TOTAL_ALLOW_TIME:720)){
      this.isAddRow = true;
      return;
    }
    this.isAddRow = false;
  ...
  validateTime(index: number = null):boolean{
    ...
    if(index != null && index >= 0){
      if(this.timesheet_detail[index].TOTAL_TIME != "" && this.timesheet_detail[index].TOTAL_TIME != null){
        current_total_min = this.total_minutes + this.timesheet_detail[index].TOTAL_MIN
      }
    } else{
    ...
    if(current_total_min > (this.TOTAL_ALLOW_TIME >0?this.TOTAL_ALLOW_TIME:720)){
      if(index != null){
        this.timesheet_detail[index].TOTAL_TIME = '';
      } else {
        this.new_data.TIME = '';
      }
      this.toast.warning('Your time exceed than working hour TIME IN to TIME OUT hours')
      return false;
    } else {
      return true;
    }
  ...
  onSubmitTimeSheet() {    
    if(this.new_data.PROJECT || this.new_data.TASK || this.new_data.TYPE || this.new_data.TIME){
      if(!this.f_addRow()){
        return;
      }
    }
    if(this.timesheet_detail.length == undefined || this.timesheet_detail.length == 0){
      this.toast.warning("No task available, Please add task.")
      return;
    }
    for(let i=0; i< this.timesheet_detail.length;i++){
      this.timesheet_detail[i].TOTAL_MIN = this.f_H_M(this.timesheet_detail[i].TOTAL_TIME)
    }
    this.spinner = true
    let data = {
      DATE: this.DATE,
    ...
    this.http.PostRequest(this.apiUrl.SaveTimeSheet, data).then(res => {
      if (res.flag) {
