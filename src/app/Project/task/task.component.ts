...
export class TaskComponent implements OnInit {
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      ...
      this.form.get('PLANNED_START_DATE').setValue(this.sharedService.getTodayDate())
      this.PLANNED_START_DATE = this.sharedService.getTodayDate();
      this.form.get('COMPLETION_DATE').setValue(this.sharedService.getTodayDate())
  ...
  GetTaskCommonList() {
    ...
    this.http.PostRequest(this.apiUrl.GetTaskCommonList, {}).then(res => {
      if (res.flag) {
  ...
  GetTaskDetail() {
    ...
    this.http.PostRequest(this.apiUrl.GetTaskDetail, data).then(res => {
      if (res.flag) {
  ...
  addRow() {
    ...
    if (this.project_assign_emp_detail.length) {
      if(this.selected_emp != ""){
        let _emp = typeof(this.selected_emp)=='object'?this.selected_emp.EMP_CODE:JSON.parse(this.selected_emp);
      ...
      this.project_assign_emp_detail.push({
        TASK_ID: this.search_task_id == ''?0:this.search_task_id,
        EMP_NO: typeof(this.selected_emp)=='object'?this.selected_emp.EMP_CODE:JSON.parse(this.selected_emp).EMP_CODE,
        FULL_NAME: typeof(this.selected_emp)=='object'?this.selected_emp.USER_NAME:JSON.parse(this.selected_emp).USER_NAME,
        ACTIVE: 1
      })
    }
    }
  ...
  saveFormData(para: string = '') {
    ...
    if (this.form.valid) {
      ...
      if(this.form.getRawValue().CLIENT_REFNO == "" || this.form.getRawValue().CLIENT_REFNO == null || this.form.getRawValue().CLIENT_REFNO == undefined)
      ...
      this.http.PostRequest(this.apiUrl.SaveTaskDetail, data).then(res => {
        if (res.flag) {
          // console.log(res);
  ...
  f_H_M(hours: string = ''): any{
   ...
   f_M_H(minutes: any = null): any{
      ...
      const M = minutes % 60;
        hour = ("0000" + H.toString()).slice(-4) + ':'  + ("00" + M.toString()).slice(-2);
      }
    return hour;
  }
  ...
  f_formValidation() {
    ...
    } else if(this.form.controls["ESTIMATED_HOURS"].invalid && this.form.getRawValue().CHARGEABLE == "1"){
