...
export class TimeSheetApproveComponent implements OnInit {
  ...
  GetPendingAttendanceList() {
    ...
    this.http.PostRequest(this.apiUrl.GetPendingAttendanceList, data).then(
      (res) => {
        if (res.flag) {
  ...
  GetPendingTimesheetList() {
    ...
    this.http.PostRequest(this.apiUrl.GetPendingTimesheetList, data).then(
      (res) => {
        if (res.flag) {
  ...
  approveRejectTimesheet(_data: any = "", index: number, type: string = "") {
    if (type == "R") {
      if (
        _data.REJECT_REASON == undefined ||
        _data.REJECT_REASON == null ||
        _data.REJECT_REASON == ""
      ) {
    ...
    this.http.PostRequest(this.apiUrl.Approve_Reject_Timesheet, data).then(
      (res) => {
        if (res.flag) {
