...
export class InvoiceRequestChangeComponent implements OnInit {
  ...
  data: { DOC_VALUE: string; TOTAL_AMOUNT_VALUE: string } = {
    DOC_VALUE: "",
    TOTAL_AMOUNT_VALUE: "",
  };
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      ...
      if (this.sharedService.loginUser[0].FYEAR == undefined) {
        this.sharedService.loginUser = sessionStorage.getItem("user_detail")
      ...
      setTimeout(() => {
        ...
        this.form
          .get("COMPANY_CODE")
          .setValue(this.sharedService.loginUser[0].COMPANY_CODE);
        this.form
          .get("LOCATION_CODE")
          .setValue(this.sharedService.loginUser[0].LOCATION_CODE);
      ...
      this.today_date_s = this.datepipe.transform(
        this.today_date,
        "yyyy-MM-dd"
      );
  ...
  filterLocations() {
    if (
      this.form.getRawValue().COMPANY_CODE != "" &&
      this.form.getRawValue().COMPANY_CODE != null
    ) {
      this._location_list = [];
      this.location_list.forEach((element) => {
        if (
          Number(element.COMPANY_CODE) ==
          Number(this.form.getRawValue().COMPANY_CODE)
        ) {
  ...
  SelectState() {
    if (
      this.form.getRawValue().LOCATION_CODE != "" &&
      this.form.getRawValue().LOCATION_CODE != null
    ) {
      this.location_list.forEach((element) => {
        if (
          Number(element.LOCATION_CODE) ==
          Number(this.form.getRawValue().LOCATION_CODE)
        ) {
  ...
  uploadDoc() {
    ...
    console.log(
      "Number of documents in uploadedDocument array:",
      this.uploadedDocument.length
    );
  }
  SaveInvoiceRequestChange() {
    ...
    for (let i = 0; i < this.uploadedDocument.length; i++) {
      if (
        this.uploadedDocument[i].ISNEW == 1 ||
        this.uploadedDocument[i].ACTIVE == 0
      ) {
        _documents.push({
    ...
    this.http.PostRequest(this.apiUrl.SaveInvoiceDetails, data).then(
      (res) => {
        if (res.flag) {
  ...
  viewList() {
    ...
    this.spinner = true;
    this.http.PostRequest(this.apiUrl.GetInvoiceViewList, data).then(
      (res) => {
        if (res.flag) {
  ...
  f_uploadFiles(fileArray: Array<any> = []) {
    ...
    if (isFileSave)
      this.http.uploadFiles(this.apiUrl.SaveFile, {
        filedata: _documents,
  ...
  GetSOCommonList() {
    this.spinner = true;
    this.http.PostRequest(this.apiUrl.GetSOCommonList, {}).then(
      (res) => {
        if (res.flag) {
          this.customer_list = res.customer_list;
          this.project_list = res.project_list;
  ...
  GetSOList() {
    ...
    this.spinner = true;
    this.http.PostRequest(this.apiUrl.GetSOList, data).then(
      (res) => {
        if (res.flag) {
          this.SO_list = res.SO_list;
          ...
          this.PendingSO_list = this.SO_list.filter(
            (item) => item.SO_STATUS === "P"
          );
  ...
  filterProject() {
    ...
    if (
      this.form.getRawValue().CUST_CODE != "" &&
      this.form.getRawValue().CUST_CODE != null
    ) {
      this._project_list = [];
      this.invoice_detail = [];
  ...
  getsonumderlist() {
    ...
    this.SO_list.forEach((element) => {
      if (
        element.CUST_NAME == this.Cust_name &&
        element.PROJ_NAME == this.Proj_name
      ) {
        this.So_NumberList.push({
          SO_NO: element.SO_NO,
          SO_REMARKS: element.SO_REMARKS,
        });
      }
    });
  ...
  ChangeBillDate() {
    this.REQ_DATE = this.datePipe.transform(
      new Date(this._DATE),
      "dd-MMM-yyyy"
    );
    //  this.REQ_DATE
  ...
  f_fillForm() {
    ...
    this.form
      .get("RAISE_INVOICE_ON")
      .setValue(this.INVOICELIST[0].RAISE_INVOICE_ON);
    ...
    this.form
      .get("LOCATION_CODE")
      .setValue(Number(this.INVOICELIST[0].LOCATION_CODE));
    this.filterLocations();
    setTimeout(() => {
      $(".selectpicker").selectpicker("refresh").trigger("change");
    }, 100);
  ...
  GetViewInvoiceReq(SO_NO: any) {
    ...
    console.log("data", data);
    this.spinner = true;
    this.http.PostRequest(this.apiUrl.GetViewInvoiceReq, data).then(
      (res) => {
        if (res.flag) {
          this.INVOICELIST = res.INVOICELIST;
          this.GetSOCommonList();
