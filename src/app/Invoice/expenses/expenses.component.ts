...
export class ExpensesComponent implements OnInit {
  ...
  LISTTYPE: any = 'P';
  SEARCH: any;
  sampel_all_expense_detail: Array<any> = [];
  ...
  GetExpenseList(val: any) {
    ...
    this.http.PostRequest(this.apiUrl.GetExpenseList, data).then(res => {
      if (res.flag) {
        ...
        this.sampel_all_expense_detail = this.all_expense_detail;
        setTimeout(() => {
          $('.selectpicker').selectpicker('refresh').trigger('change');
        }, 100);
        this.spinner = false;
  ...
  SearchFilter(val: any) {
    if (!val || val.trim() === '') {
      this.all_expense_detail = [...this.sampel_all_expense_detail];
      return;
    }
    const searchTerm = val.toLowerCase();
    this.all_expense_detail = this.sampel_all_expense_detail.filter((item: any) =>
