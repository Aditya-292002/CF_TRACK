...
export class SalesOrderCreateComponent implements OnInit {
 ...
 @ViewChild('CheckValidationForInvoiceCreate', { static: false }) modal: ElementRef;
  ...
  constructor(public sharedService: SharedServiceService,
    ...
    public currencyPipe: CostFilterPipe) { }
...
this.raisedinvoiceonmaxDate = currentDate.toISOString().split('T')[0];
  ...
  SaveSalesOrderRelease(val:any) {
    ...
    //   if (!this.sharedService.isValid(element.REQ_VALUE)) {
    //     this.toast.error('Enter a Req Value');
    //     return false;
    //   }
    // }
    ...
    for (const element1 of this.SO_MILESTONE_T) {
      ...
      if((element1.DOC_VALUE - element1.BILLED_VALUE) < element1.REQ_VALUE){
        this.toast.error('Invalid req value should not be add request');
        return
      }
    }
    ...
    this.SO_MILESTONE_T.forEach((element:any)=>{
      if ((+this.currencyPipe.parse(element.REQ_VALUE)) > 0 || element.IS_CLOSED == true )  {
        SO_MILESTONE_T.push(element);   
      }
    })

    SO_MILESTONE_T.forEach((element:any)=>{
      ...
      // return  
      this.http.PostRequest(this.apiUrl.SaveSalesOrderRelease, data).then(res => {
        if (res.flag == 1) {
  ...
  removeData(data: any): void {
    ...
    this.SO_MILESTONE_T.forEach((element:any)=>{
      TOTAL_AMOUNT_VALUE += element.BILLED_VALUE
    })
